...
export const NetlifyLogo = ({ className }) => (
...
export const WizardProgressBar = ({ value }) => {
  return (
    <div className="w-full relative">
      <div
        className="bg-primary h-1 absolute top-0 left-0"
        style={{ width: `${(value / TOTAL_STEPS) * 100}%` }}
      />
    </div>
  );
};

export const WizardContent = ({ children, step }) => (
  <div className="text-center bg-white bg-opacity-60 dark:bg-black dark:text-white dark:bg-opacity-30 backdrop-blur-lg rounded-b-xl max-w-4xl w-full border border-gray-800 dark:border-white border-opacity-10 dark:border-opacity-10 my-12">
    <WizardProgressBar value={step + 1} />
...
export const WizardLabel = ({ as: Component = "p", children, ...rest }) => (
  <Component
    className="block uppercase mb-2 text-gray-700 font-bold dark:text-white dark:opacity-60"
    {...rest}
  >
    {children}
  </Component>
...
export const WizardInput = ({ label, id, ...rest }) => (
  <div className="text-left">
    {label && id && (
      <WizardLabel htmlFor={id} as="label">
        {label}
      </WizardLabel>
    )}
    <input
      className="rounded-xl w-full text-gray-700 dark:text-white dark:bg-gray-700 dark:border-gray-500 border border-gray-300 p-4 shadow-md"
      id={id}
      {...rest}
    />
  </div>
...
export const WizardButton = ({
  ...
  return (
    <Component className={classes} {...rest}>
      {children}
      {icon && <div className="ml-2 w-6 h-6 inline-block">{icon}</div>}
    </Component>
  );
};

export const WizardPreheader = ({ children }) => (
  <p className="-mt-2 mb-4 text-gray-800 dark:text-white dark:text-opacity-60">
    {children}
  </p>
);
